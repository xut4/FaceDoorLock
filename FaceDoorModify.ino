...
face_id_name_list st_face_list; // 用來存人臉資料的
...
void setup() {
  ...
  if (psramFound()) {
    ...
    Serial.print("Total PSRAM available: ");
    Serial.println(ESP.getPsramSize());  
  int *data = (int *)heap_caps_malloc(1024 * 1024, MALLOC_CAP_SPIRAM);
  if (data != NULL) {
...
void handle_message(WebsocketsClient &client, WebsocketsMessage msg)
{
  ...
  if (msg.data().substring(0, 8) == "capture:") {
    ...
    memcpy(st_name.enroll_name, person, strlen(person) + 1);  // 註冊人名
...
void loop() {
  ...
  while (client.available()) {
    ...
    fb = esp_camera_fb_get(); // 拿到照片

    if (g_state == START_DETECT || g_state == START_ENROLL || g_state == START_RECOGNITION)
    {
      out_res.net_boxes = NULL; // 清空所抓到的人臉框資料
      out_res.face_id = NULL; // 清空所放的人臉資調

      fmt2rgb888(fb->buf, fb->len, fb->format, out_res.image); // 都轉成rgb888

      out_res.net_boxes = face_detect(image_matrix, &mtmn_config); // 用來檢測人臉 返回人臉框

      if (out_res.net_boxes)
      {
        if (align_face(out_res.net_boxes, image_matrix, aligned_face) == ESP_OK) // 將檢測到的人臉進行對齊（Alignment），以提高辨識準確性
        {

          out_res.face_id = get_face_id(aligned_face); // get_face_id用來拿人臉特徵
          last_detected_millis = millis(); // 用來記錄當前時間
